package com.example.myspace2.Individual;
...
public class Note implements NoteDataBaseHelper.TableCreateInterface {
    ...
    public static NoteDataBaseHelper.TableCreateInterface getIntance() {
        return note;
    }
    ...
    public static void deleteNote(NoteDataBaseHelper dataBaseHelper, int _id){
        SQLiteDatabase db = dataBaseHelper.getWritableDatabase();
        db.delete(Note.tableName,Note._id + "=?",new String[]{_id+ ""});
        db.close();
    }
    public static void updateNote(NoteDataBaseHelper dataBaseHelper, int _id, ContentValues infoValues){
        SQLiteDatabase db = dataBaseHelper.getWritableDatabase();
